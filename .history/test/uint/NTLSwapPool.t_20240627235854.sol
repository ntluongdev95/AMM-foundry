...
contract NTLSwapPoolTest is Test {
    ...
    function test_DepositWithExistingLiquidity () public{
        ...
        poolToken.approve(address(pool), expectedpoolTokensToDeposit);
        pool.deposit(5e18, 5e18, 10e18, uint64(block.timestamp));
        console.log(weth.balanceOf(address(pool))); //105e18
        vm.stopPrank();
    }
    ...
    function test_revertIfDeadlinePassed ()public{
        ...
        vm.expectRevert(abi.encodeWithSelector(
           NTLSwapPool.NTLSwapPool__DeadlineHasPassed.selector,
            uint64(1)
        ));
        pool.withdraw(100e18, 100e18,10e18, uint64(1));
    }

    function testRevertIfWethToWithdrawTooLow() public {
    uint256 liquidityTokensToBurn = 1 * 10 ** 18;
    uint256 minWethToWithdraw = 10 * 10 ** 18;  // Set too high to trigger revert
    uint256 minPoolTokensToWithdraw = 1 * 10 ** 18;
    uint64 deadline = uint64(block.timestamp + 1 hours);

    vm.expectRevert(
        abi.encodeWithSelector(
            NTLSwapPool__OutputTooLow.selector,
            (liquidityTokensToBurn * MockERC20(wethToken).balanceOf(address(swapPool))) / swapPool.totalLiquidityTokenSupply(),
            minWethToWithdraw
        )
    );
    swapPool.withdraw(liquidityTokensToBurn, minWethToWithdraw, minPoolTokensToWithdraw, deadline);
}
