...
contract NTLSwapPool is ERC20 {
    ...
    modifier revertIfDeadlinePassed(uint64 deadline) {
     ...
     modifier revertIfNotEnoughLPToken(uint256 amount) {
        if ( < block.timestamp) {
            revert NTLSwapPool__DeadlineHasPassed(deadline);
        }
        _;
    ...
    */
    ...
    {   
        ...
        if (poolTokensToWithdraw < minPoolTokensToWithdraw) {
            revert NTLSwapPool__OutputTooLow(poolTokensToWithdraw, minPoolTokensToWithdraw);
        }
    ...
    function swapExactInput(
        ...
        uint64 deadline
    ) public revertIfZero(inputAmount) revertIfDeadlinePassed(deadline) returns (uint256 outputAmount) {
